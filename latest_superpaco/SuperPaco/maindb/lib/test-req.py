...
#SuperPaco Tests
r = requests.post("http://127.0.0.1:8000/back_end/basic_data/",headers = {'Content-Type' : 'application/json' }, data = json.dumps({"godini" : "15", "gender" : "Maski", "zrtva_na_nasilstvo" : "da", "svedok_na_nasilstvo" : "da" }))
print(r.text)
z = 0
while True:
   q = requests.post("http://127.0.0.1:8000/back_end/id_check/",headers = {'Content-Type' : 'application/json' }, data = json.dumps({"id" : r.text, "choices" : "[1, 2, 1, 3]", "bad_choices" : "2", "good_choices" : "3" }))
   print(q.text)
   z+=1
   if z == 4:
      break
